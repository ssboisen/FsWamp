...
let main argv = 
    ...
    let s = atom 5
    let k = Guid.NewGuid()
    addWatch k (fun k o n -> printfn "watcher '%A' notified of value change from '%i' to '%i'" k o n) s
    printfn "state: %i" !s
    let b = swap (fun v -> v + 1) s
    printfn "ret: %i" b
    let r = compareAndSet 5 8 s
    printfn "ret: %A" r
    let r = compareAndSet 6 8 s
    printfn "ret: %A" r
    removeWatch k s
    let r = set 9 s
    printfn "ret: %A" r
